                    ...
                    unsigned char *output )
{
    ...
    if(length > 0)
    {
        ...
        if(mode == AES_DECRYPT)
        {
            ...
            rtl_crypto_aes_cbc_decrypt(input, length, iv_buf_aligned, 16, output_buf_aligned);
            memcpy(iv, iv_tmp, 16);
        }
        else
        {
            memcpy(key_buf_aligned, ctx->enc_key, ((ctx->nr - 6) * 4));
            rtl_crypto_aes_cbc_init(key_buf_aligned,((ctx->nr - 6) * 4));
            rtl_crypto_aes_cbc_encrypt(input, length, iv_buf_aligned, 16, output_buf_aligned);
